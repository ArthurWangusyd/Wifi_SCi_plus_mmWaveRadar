...
rangeBinEndIndex=10; %我们舍弃前两个传回的信号，认为他们在处理时间上的延迟大于距离0
for k=1:1:frame
...
%计算信号相位
for k=1:frame
    signal_phase(:,k)=atan(data_imag(:,k)/data_real(:,k)); %其实直接用angle也行
end
...
for k=2:frame
    ...
    elseif diff<-pi/2
        signal_phase(:,(k:end))=signal_phase(:,(k:end))+pi; %这一步是结卷，将差距限制在-pi/2到pi/2之间，我们只观察相位变化，因为我们认为呼吸心跳产生的只有相位变化
    end
end

%计算相位差
for k=1:frame-1
    delta_phase(:,k)=signal_phase(:,k+1)-signal_phase(:,k);%计算相位差
end

%从波形中去除脉冲噪声
thresh=0.8;
for k=1:frame-3 
    phaseUsedComputation(:,k)=filter_RemoveImpulseNoise(delta_phase(:,k),delta_phase(:,k+1),delta_phase(:,k+2),thresh); %去除脉冲噪声
end
index=1:1:frame-3;
index=index*Tf;
